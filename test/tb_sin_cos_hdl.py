...
class TestHardVit(unittest.TestCase):
    def test_sweep(self):
        def test(clk,
            ...
            theta, ref_x, ref_y = direte_comp_test()

            while True:
                for ii in range(len(theta)):
                    in_vld.next = 1
                    in_z.next = int(theta[ii])
                    while True:
                        yield clk.posedge
                        in_vld.next = 0
                        if out_vld == 1:
                            self.assertEqual(out_x, ref_x[ii])
                            self.assertEqual(out_y, ref_y[ii])
                            break
    ...
    def runTests(self, test):
        ...
        in_z = Signal(intbv(0, min=-2**(phase_wid-1), max=2**(phase_wid-1)))
